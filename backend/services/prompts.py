"""
CarbonFactors — LLM Prompt Templates
...
def alternative_suggestion_prompt(activity_json: str, rag_chunks: str) -> str:
    return f"""You are CarbonFactors, a climate advisor. The user just logged a carbon-emitting activity.
...
def insights_prompt(
    ...
    return f"""You are CarbonFactors. Generate personalised carbon footprint insights for this user.
...
def weekly_report_prompt(
    ...
    return f"""You are CarbonFactors. Generate a weekly carbon footprint digest.
...
def rag_qa_prompt(question: str, rag_chunks: str) -> str:
    return f"""You are CarbonFactors, a climate knowledge assistant.
